 ...
 * ============================================================================
 * $Author: luozhibiao $
 * $Id: user.php 17217 2011-01-19 06:29:08Z luozhibiao $
 */
define('IN_ECS', true);
...
/* 代码增加_Start By uppschina.com  用户中心获取会员信息*/
...
function action_default ()
{
	...
	$ecs = $GLOBALS['ecs'];
	$user_id = $_SESSION['user_id'];
	/* 代码注释 by uppschina.com _star 路由前已调用会员信息，去掉重复调用*/
	$rank = get_rank_info();
	/* 	include_once (ROOT_PATH . 'includes/lib_clips.php');
	 if($rank = get_rank_info())
	 {
	...
	/* 代码增加--cb--推荐分成-- by uppschina.com _star */
	$rn = $rank['rank_name'];
	$recomm = $db->getOne("SELECT is_recomm FROM " . $GLOBALS['ecs']->table('user_rank') . " WHERE rank_name= '$rn'");

    $smarty->assign('recomm', $recomm); // 获取当前用户是否是分成用户判断是否显示我的推荐
	/* 代码增加--cb--推荐分成-- by uppschina.com _end */
...
//	$order['shipping_status'] = $_LANG['ss'][$order['shipping_status']];
	...
	foreach($goods_list as $goods_key => $goods_val)
	{
		...
		/* 代码增加_start By uppschina.com _20150804 */
		...
		/* 代码增加_end By uppschina.com _20150804 */
