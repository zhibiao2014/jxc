...
if ($_REQUEST['step'] == 'add_to_cart')
{
	...
	//uppschina.com start add 2015-3-26
	$time_xg_now=gmtime();
	...
	//uppschina.com end add 2015-3-26
	...
	$result['goods_id'] = $goods->goods_id;
	$sql_where = $_SESSION['user_id']>0 ? "user_id='". $_SESSION['user_id'] ."' " : "session_id = '" . SESS_ID . "' AND user_id=0 ";//添加 uppschina.com
...
/* 代码增加_end  By uppschina.com */
    ...
    else
    {
        ...
        elseif (!empty($_POST['act']) && $_POST['act'] == 'signup')
        {
            ...
            {
                /* 用户注册成功 */
				/* 四合一修改 start by uppschina.com */
                ecs_header("Location: flow.php?step=checkout\n");
				/* 四合一修改 end by uppschina.com */
                exit;
            }
            else
            {
...
elseif ($_REQUEST['step'] == 'checkout')
{
   ...
   // else if($_SESSION['extension_code'] == 'virtual_good')
    ...
    {
        /* 如果不完整则转向到收货人信息填写界面 */
		/* 代码修改_start  By  uppschina.com  注释或删掉  */
        //ecs_header("Location: flow.php?step=consignee\n");   
        //exit;
...
elseif ($_REQUEST['step'] == 'select_payment')
{
    ...
    else
    {
		...
		/*余额额支付密码_添加_START_uppschina.com*/
        $order['pay_code'] = $payment_info['pay_code'];
	    /*余额额支付密码_添加_END_uppschina.com*/
...
elseif ($_REQUEST['step'] == 'change_surplus')
{
    ...
    else
    {
        ...
        else
        {
            ...
            /* 代码修改 By  uppschina.com #1305 Start */
//            if($total['amount_formated'] <= 0){
            if($total['amount'] <= 0)
            {
            /* 代码修改 By  uppschina.com #1305 End */
    ...
    /* 代码修改 By  uppschina.com #1854 Start */
    $result['surplus'] = round($result['surplus'] ,2);
    /* 代码修改 By  uppschina.com #1854 End */
    $json = new JSON();
    die($json->encode($result));
}
...
//                    }
                    ...
                    /* 代码增加_end  By uppschina.com _sunlizhi*/
...
{
   /* 代码修改_start  By  uppschina.com  将这块替换掉*/
...
{
    /* 查询：购物车中所有不可以单独销售的配件 */
	$sql_where = $_SESSION['user_id']>0 ? "c.user_id='". $_SESSION['user_id'] ."' " : "c.session_id = '" . SESS_ID . "' AND c.user_id=0 ";//添加 uppschina.com
