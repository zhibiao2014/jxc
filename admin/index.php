 ...
 * QFYU 控制台首页
 * ============================================================================
 * 版权所有 2016 深圳市联合精准农业服务科技有限公司，并保留所有权利。
...
{
    clear_all_files();
    clearhtml_all();  //代码增加   By  uppschina.com
    echo 1;
    //sys_msg($_LANG['caches_cleared']);
}
//uppschina.com手机缓存修改start
...
elseif ($_REQUEST['act'] == 'main')
{
    ...
    /*后台管理起始页_修改_START_uppschina.com*/
...
//    " WHERE supplier_id=0  " . order_query_sql('await_pay'));
    ...
    $order['new_repay'] = $db->getOne('SELECT COUNT(*) FROM '.$ecs->table('back_order').' WHERE status_back < 6 AND status_back != 3 AND back_type = 4 AND supplier_id=0');// 代码修改   By  uppschina.com End
    //退货申请
    // 代码修改   By  uppschina.com Start
...
//         $api_comment = $t->request('http://api.68ecshop.com/census.php', $apiget);
        $api_comment = array(
        		"header"=>"HTTP/1.1 200 OK Server: Nginx Date: Mon, 01 Aug 2016 04:03:47 GMT Content-Type: text/html Connection: close X-Powered-By-360WZB: wangzhan.360.cn",
        		"body"=>'{"ver":"v1.0.1","name":" xjd_dan ","lang":" zh_cn ","release":" 20150923 ","php_ver":" 5.6.20 ","mysql_ver":" 5.7.9 ","ocount":" 171 ","oamount":" 107149.08 ","gcount":" 226 ","charset":" UTF-8 ","usecount":" 56 ","template":" 68ecshopcom_360buy ","style":" 0 ","url":" http:\/\/192.168.0.112\/ ","ip":" 192.168.0.112 ","type":" 1 ","patch":" 27320130308008 ","version":{"ver":"庆丰裕 v1.0.1","down":""}}'	
        );	
