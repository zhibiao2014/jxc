...
class cls_session
{
    ...
    function cls_session(&$db, $session_table, $session_data_table, $session_name = 'ECS_ID', $session_id = '')
    {
        ...
        {
            $this->session_id = $GLOBALS["_COOKIE"]['ECSCP_ID'];//2016-08-01 后台同步登录 Auth: zhibiao $
        }
        if ($this->session_id)
        {
...
//         	var_dump($this->session_id);exit();
        ...
        else
        {
            ...
            setcookie($this->session_name, $this->session_id . $this->gen_session_key($this->session_id), time()+86400*7, $this->session_cookie_path, $this->session_cookie_domain, $this->session_cookie_secure); //代码修改 By  uppschina.com  
        }
    ...
    function gen_session_key($session_id)
    {
        ...
        return sprintf('%08x', crc32(ROOT_PATH. 'mobile/' . $ip . $session_id));//2016-08-01 后台同步登录 Auth: zhibiao $
    }
    ...
    function destroy_session()
    {
		...
		/* 注释掉下面这段代码或者直接删除_start   By  uppschina.com */		
